...
const JSONBuffer = require("./JSONBuffer");
...
class ReziDB {
  constructor(
    ...
    if (!existsSync(`${options["path"]}/${options["name"]}`))
      mkdirSync(`${options["path"]}/${options["name"]}`);
  ...
  async forEach(callback) {
    try {
      const stream = this.database.createReadStream({
        keys: true,
        values: true,
      });
  ...
  async size() {
    try {
      return new Promise((resolve, reject) => {
        ...
        this.database
          .createKeyStream({ keys: false, values: false })
